...
class RoomScreen extends StatefulWidget {
  ...
  // --- ✅ 2. Provider 제공 수정 (chatRoomId! 사용 및 null 체크) ---
  /// RoomScreen을 생성할 때 NpcProvider와 ChatService를 함께 제공
  static Widget create({required Room room}) {
    ...
    // ✅ 1. chatRoomId null 체크 추가
    // 백엔드에서 chatRoomId를 받아오지 못하면 채팅 서비스를 시작할 수 없음.
    ...
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(
          // ✅ 2. room.id! (String) 대신 room.chatRoomId! (int) 전달
          create: (_) => ChatService(room.chatRoomId!), // 채팅방의 숫자 ID 전달
        ),
        // TODO: VttSocketService도 이곳에서 주입해야 합니다.
        // ChangeNotifierProvider(
        //   create: (_) => VttSocketService(room.id!)..connectAndJoin('DEFAULT_MAP_ID'),
        // ),
      ],
  ...
  // byId 생성자도 RoomScreen.create를 사용하도록 수정 (기존과 동일)
...
class RoomScreenState extends State<RoomScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initializeScreen() async {
    ...
    // NpcProvider와 ChatService는 RoomScreen.create에서 생성될 때 자동으로 초기화됨
  }
  // --- ✨ ---
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      // ✅ ChatService의 재연결은 ChatService 내부에서 처리 (socket.io)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // --- ✅ 3. Body 수정: VTT와 채팅 UI 함께 표시 ---
      body: Consumer<NpcProvider>(
          // NpcProvider 상태 변화 감지 (VTT/NPC 로딩 등에 사용)
          builder: (context, npcProvider, child) {
        ...
        // VTT Canvas와 ChatListWidget을 Stack으로 겹칩니다.
        return Stack(
          children: [
            // --- VTT Canvas 영역 (구현 필요) ---
            // TODO: Positioned.fill(child: VttCanvasWidget()), // VttCanvas 위젯
            const Center(child: Text('VTT Canvas 영역')), // 임시 텍스트

            // --- 채팅 UI 영역 (새로 추가) ---
            // ChatListWidget은 Consumer<ChatService>를 내장하고 있습니다.
            // ChatService는 static Widget create()에서 주입되었습니다.
            ChatListWidget(
              participants: _participants, // 현재 참여자 목록 전달
              currentUserId: _currentUserId, // 현재 사용자 ID 전달
            ),
          ],
        );
      }),
  ...
  void _handleSendChat() {
    ...
    try {
      ...
      // (선택) ChatService의 sendMessage가 예외를 던질 경우 (예: 로그아웃 상태)
